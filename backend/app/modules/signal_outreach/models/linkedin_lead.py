...
class DmGenerationStatus:
    """
    Constants for DM generation status.
    Tracks the AI-powered DM generation lifecycle.
    
    Using plain class with string constants instead of Enum to avoid
    SQLAlchemy/PostgreSQL enum mapping issues.
    """
    PENDING = "pending"      # DM generation queued/in-progress
    GENERATED = "generated"  # DM successfully generated by AI
    FAILED = "failed"        # DM generation failed after retries
...
class LinkedInLead(Base):
    ...
    # ============================================
    # Tracks AI-powered DM generation status for async processing
    # Values: 'pending', 'generated', 'failed' (see DmGenerationStatus class)
    dm_generation_status = Column(
        Text,
        default=DmGenerationStatus.PENDING,
        nullable=False,
    ...
    dm_generation_started_at = Column(
        ...
        nullable=True
    )  # When DM generation was initiated (for stuck detection)
    ...
    __table_args__ = (
        ...
        Index('idx_linkedin_leads_dm_gen_status', 'dm_generation_status'),  # For background task queries
    )
